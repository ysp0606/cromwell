...
class FileEvaluatorSpec extends FlatSpec with Matchers {
  val expressions = Table(
    ...
    (""" ["a", "b", "c"] """, Seq(WomSingleFile("a"), WomSingleFile("b"), WomSingleFile("c")), WomArrayType(WomSingleFileType)),
    ...
    (""" [fileNameAsStringInput, "${fileNameAsStringInput}.bai"] """, Seq(WomSingleFile("sommat.bam"), WomSingleFile("sommat.bam.bai")), WomArrayType(WomSingleFileType)),
    (""" [ fileNameAsStringInput, mapToFileName["Chris"] ] """, Seq(WomSingleFile("sommat.bam"), WomSingleFile("sommatStupid.bam")), WomArrayType(WomSingleFileType)),
    ...
    (""" if read_int("i.txt") == 10 then "a.txt" else "b.txt" """, Seq(WomSingleFile("a.txt"), WomSingleFile("b.txt"), WomSingleFile("i.txt")), WomSingleFileType),
