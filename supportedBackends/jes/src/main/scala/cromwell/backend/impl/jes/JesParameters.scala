...
object JesInput {
  ...
  lazy val defaultGcsPrefix = ConfigFactory.load().as[Option[String]]("papi.default-input-gcs-prefix") map {
    p => if (p.endsWith("/")) p else p + "/"}

  private[jes] def prefixIfNecessary(url: String): String = {
    val prefix = (for {
      p <- defaultGcsPrefix
