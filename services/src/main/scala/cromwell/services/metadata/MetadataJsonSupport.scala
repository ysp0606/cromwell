...
object MetadataJsonSupport extends DefaultJsonProtocol {
  implicit object WorkflowIdJsonFormatter extends RootJsonFormat[WorkflowId] {
    ...
    def read(value: JsValue) = throw new UnsupportedOperationException("Reading WorkflowId from JSON is unsupported")
  }

  implicit object MetadataTypeFormatter extends RootJsonFormat[MetadataType] {
    def write(t: MetadataType) = JsString(t.typeName)
    def read(value: JsValue) = throw new UnsupportedOperationException("Reading MetadataType from JSON is unsupported")
  }
  ...
  implicit object OffsetDateTimeFormatter extends RootJsonFormat[OffsetDateTime] {
    ...
    def read(value: JsValue) = throw new UnsupportedOperationException("Reading OffsetDateTime from JSON is unsupported")
  }
