...
class StandardExpressionFunctions(val standardParams: StandardExpressionFunctionsParams)
  ...
  override def stdout(params: Seq[Try[WomValue]]): Try[WomFile] = Success(WomSingleFile(callContext.stdout))

  override def stderr(params: Seq[Try[WomValue]]): Try[WomFile] = Success(WomSingleFile(callContext.stderr))
}
